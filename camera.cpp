...
//	m_camera.posV = D3DXVECTOR3(CAMERA_POSV_X, CAMERA_POSV_Y, CAMERA_POSV_Z);
	...
	m_camera.rot = D3DXVECTOR3(0.0f, 0.9999999f, 0.0f);
	m_camera.fDistance = D3DXVECTOR3(CAMERA_DISTANCE_X, CAMERA_DISTANCE_Y, CAMERA_DISTANCE_Z);
	m_camera.posR_y = 0.0f;
	m_camera.posV_y = CAMERA_POSVY_MIN;
	m_camera.newRot = 0.0f;
	m_camera.rotTime = 0;

	m_camera.posV = D3DXVECTOR3(m_camera.posR.x + sinf(D3DX_PI * 0.0f) * CAMERA_DISTANCE_Z, CAMERA_POSV_Y, -(m_camera.posR.z - cosf(D3DX_PI * 0.0f) * CAMERA_DISTANCE_Z) );
}
...
void CCamera::Update(void) {
	...
	MODEL model = get_player -> GetModel( );
...
/*		// 平行移動
	...
	if(input -> GetKeyboardPress(DIK_Z) || input -> GetKeyboardPress(DIK_C) || input -> GetKeyboardPress(DIK_I) || input -> GetKeyboardPress(DIK_O) || input -> GetKeyboardPress(DIK_U) || input -> GetKeyboardPress(DIK_M) || input -> GetKeyboardPress(DIK_Y) || input -> GetKeyboardPress(DIK_N) ){
		// 視点操作
		// 旋回
		if(input -> GetKeyboardPress(DIK_C) || input -> GetKeyboardPress(DIK_O) ) {
			m_camera.rot.y -= CAMERA_ROT_SPEED;
		}
		else if(input -> GetKeyboardPress(DIK_Z) || input -> GetKeyboardPress(DIK_I)) {
			m_camera.rot.y += CAMERA_ROT_SPEED;
		}
	...
	// テスト
	// 地形回避処理 プレイヤーとカメラの間にフィールドが存在してプレイヤーが見えない場合、カメラをオート移動させてプレイヤーを画面内に収める
	float height;
	D3DXVECTOR3 pos, vec;
	bool chk = false;
	bool chk2 = false;

/*	vec = D3DXVECTOR3(m_camera.posV.x, m_camera.posV.y + m_camera.posV_y, m_camera.posV.z);
	vec = (model.pos - vec) * 0.1f;*/
	vec = (model.pos - m_camera.posV) * 0.1f;
	for(int i = 1; i < 10; i++) {
		pos = m_camera.posV + vec * (float)i;
		height = CMeshfield::GetFieldHeight(pos);
		if(pos.y - 0.5f < height) {
			chk = true;
			if(pos.y < height) {
				m_camera.posV_y += 0.5f;
//				if(m_oldCameraPosV.y >= height) {
				if(m_oldCameraPosV.y > m_camera.posV.y) {
					m_camera.posV_y = m_camera.posV.y - m_camera.fDistance.y - model.pos.y;
				}
				break;
			}
		}
		CScene::HitCheckAll(pos, D3DXVECTOR3(0, 0, -12345.1f) );
	}
	...
	if(!chk && m_camera.posV_y > CAMERA_POSVY_MIN) {
		m_camera.posV_y -= 0.5f;
	}

	m_oldCameraPosV = m_camera.posV; // カメラ位置保存
